-- Olympic Runner (c)2020 Marco Bruti
...
function showScoreboard(cnt,time,stage)
  ...
  if (stage==gameStages["RUN"]) then  
    love.graphics.printf("GO!",0,myCanvas:getHeight()*0.70,myCanvas:getWidth()/2,"center",0,2,2)
  end  
  if (stage==gameStages["RUN"]) or (stage==gameStages["STOP"]) then
    love.graphics.printf(string.format("TIME: %06.2f s",time),0,myCanvas:getHeight()*0.80,myCanvas:getWidth()/2,"center",0,2,2)
  ...
  if (stage==gameStages["STOP"]) then
    ...
    love.graphics.printf(string.format("BEST TIME: %06.2f s",record),0,myCanvas:getHeight()*0.9,myCanvas:getWidth()/2,"center",0,2,2)
  end
end
